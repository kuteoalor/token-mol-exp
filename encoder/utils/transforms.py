    ...
    def __call__(self, data:ProteinLigandData):
        ...
        if has_unmask_atoms:
            ...
            if assign_index.size(1) == 0:
                dist = torch.norm(data.pkt_node_xyz.unsqueeze(1) - data.ligand_masked_pos.unsqueeze(0), p=2, dim=-1)
...
class Res2AtomComposer(object):  #protein_dim=26, ligand_dim=13
    ...
    def __call__(self, data):
        ...
        protein_pos = data.pkt_node_xyz
        protein_atom_feature = data.pkt_node_sca
        ...
        # expansion ligand feature dim to match the dims of protein
        ...
        ligand_context_vec_full = data.ligand_context_pos
        protein_vec = data.pkt_node_vec
        ligand_context_vec_full_expand = torch.cat([ligand_context_vec_full.unsqueeze(-2), torch.zeros(len_ligand_ctx, 2, 3)], dim=1)
        data.compose_vec = torch.cat([ligand_context_vec_full_expand, protein_vec], dim=0)
    ...
    def get_knn_graph(data, knn, len_ligand_ctx, len_compose, num_workers=1):
        ...
        # ligand_context_bond_type=1, 做到第二个维度上
...
class ContrastiveSample(object):
    ...
    def __call__(self, data:ProteinLigandData):
        ...
        if len(data.ligand_context_pos) != 0: # all mask
            ...
            pos_fake_mode = data.pkt_node_xyz[data.y_protein_frontier]
